...
export async function POST(req: NextRequest) {
  ...
  const worker = await prisma.worker.create({ data: { firstName: candidate.firstName, lastName: candidate.lastName, phone: candidate.phone, email: (candidate as any).email ?? undefined, candidateId: candidate.id, status: "active", createdById: session.user.id } });
  await prisma.candidate.update({ where: { id: candidateId }, data: { convertedWorkerId: worker.id, stage: "hired" } as any });
