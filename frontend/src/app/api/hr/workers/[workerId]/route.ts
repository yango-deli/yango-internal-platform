...
const INCLUDE = { department: true, store: true, manager: { select: { id: true, firstName: true, lastName: true } }, previousPositions: { orderBy: { order: "asc" as const } }, equipment: { where: { status: "assigned" } } };
...
export async function PATCH(req: NextRequest, { params }: { params: { workerId: string } }) {
  ...
  for (const field of TRACKED) {
    const old = (existing as any)[field]; const next = updateData[field];
    if (next !== undefined && old !== next) await logWorkerActivity({ workerId: params.workerId, userId: session.user.id!, type: field === "status" ? "status_changed" : "field_updated", description: `${field === "status" ? "status_changed" : "field_updated"}:${field}`, fieldName: field, oldValue: String(old ?? ""), newValue: String(next ?? "") });
  }
  return NextResponse.json(worker);
}
