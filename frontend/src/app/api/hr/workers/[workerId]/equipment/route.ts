...
export async function GET(_req: NextRequest, { params }: { params: { workerId: string } }) {
  ...
  return NextResponse.json(await prisma.workerEquipment.findMany({ where: { workerId: params.workerId }, orderBy: { createdAt: "desc" } }));
}
...
export async function POST(req: NextRequest, { params }: { params: { workerId: string } }) {
  ...
  await logWorkerActivity({ workerId: params.workerId, userId: session.user.id!, type: "equipment_assigned", description: `equipment_assigned:${data.name}`, metadata: { equipmentId: eq.id } });
