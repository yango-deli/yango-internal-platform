...
export async function POST(req: NextRequest, { params }: { params: { workerId: string } }) {
  ...
  if (replacesId) {
    ...
    if (old) {
      await prisma.workerDocument.update({ where: { id: replacesId }, data: { isLatest: false } });
  ...
  await logWorkerActivity({
    ...
    type: replacesId ? "document_replaced" : "document_uploaded",
    description: replacesId ? `document_replaced:${title}:${version}` : `document_uploaded:${title}`,
    metadata: { documentId: doc.id, type, version },
  });
